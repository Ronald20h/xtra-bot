...
[CONFIG.DB_PATH, path.join(__dirname,'Json-db'), path.join(__dirname,'sessions')].forEach(p => {
...
if (!fs.existsSync(PROTECT_PATH)) fs.writeFileSync(PROTECT_PATH, '{}');
if (!fs.existsSync(path.join(__dirname,'Json-db','prefix.json'))) fs.writeFileSync(path.join(__dirname,'Json-db','prefix.json'),'{}');
// مجلد لحفظ الـ sessions على الـ disk (تبقى حتى لو السيرفر اتعيد)
...
app.use(session({
  store: new FileStore({
    path: SESSION_DIR,
    ttl: 10 * 60,          // 10 دقائق بالثواني
    retries: 1,
    reapInterval: 60,      // تنظيف كل دقيقة
  }),
  secret: CONFIG.SESSION_SECRET,
  resave: true,
  saveUninitialized: false,
  rolling: true,            // يجدد الـ 10 دقائق كل ما المستخدم يعمل أي طلب
  cookie: {
    secure: false,
